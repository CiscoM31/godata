...
type MongoQuery struct {
	StringQuery string // The string representation of the Mongo query
}
...
func (p *MongoGoDataProvider) BuildQuery(r *godata.GoDataRequest) (MongoQuery, error) {
	...
	q, err := recursiveBuildWhere(r.Query.Filter.Tree)
	if err != nil {
		return MongoQuery{}, err
	}
	return MongoQuery{StringQuery: q}, err
}

func (p *MongoQuery) GetQuery() (bson.M, error) {
	var ret bson.M
	err := json.Unmarshal([]byte(p.StringQuery), &ret)
	if err != nil {
		return nil, err
	}
	return ret, nil
